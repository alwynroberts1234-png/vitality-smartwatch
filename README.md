...
2. In VS Code, select **File > Open Folder** and choose the folder containing
   this README and `.vscode/tasks.json`.
...
**Vitality: Open project** runs four tasks in sequence:

1. **Detect OS** — report Windows, Linux or macOS and use its native setup script.
2. **Check environment** — report available and missing tools without installing
   anything. Missing tools allow the next stage to proceed.
3. **Download environment** — install missing host prerequisites, prepare
   `.tools/venv` with west/CMake/Ninja, fetch Zephyr v4.1.0 and its dependencies,
   and find or install the ARM SDK. A ready environment is reused.
4. **Compile and show design** — open the animated preview and start the firmware
   build. This stage does not install dependencies.

The sequence is defined directly in `.vscode/tasks.json` with
`dependsOrder: "sequence"`. An error stops subsequent stages. All four stages
are also available individually under **Tasks: Run Task**; use **Vitality: Open
project** to run the complete flow.
